 ...
 */
static int zl3073x_hwreg_do_op(struct zl3073x_dev *zldev, unsigned char op)
{
#define ZL3073X_HWREG_TIMEOUT_MS 1000 /* 1 second */
	unsigned long timeout;
	int rc;

	/* Set requested operation and set pending bit */
	rc = zl3073x_write_hwreg_op(zldev, op | ZL3073X_REG_HWREG_OP_PENDING);
	if (rc)
		return rc;

	/* Set timeout to 1 second */
	timeout = jiffies + msecs_to_jiffies(ZL3073X_HWREG_TIMEOUT_MS);

	/* Poll for completion - pending bit cleared */
	do {
		rc = zl3073x_read_hwreg_op(zldev, &op);
		if (rc)
			return rc;

		if (op & ZL3073X_REG_HWREG_OP_PENDING)
			msleep(1);
	} while ((op & ZL3073X_REG_HWREG_OP_PENDING) &&
		 time_before(jiffies, timeout));

	return (op & ZL3073X_REG_HWREG_OP_PENDING) ? -ETIMEDOUT : 0;
}
 ...
 */
static int zl3073x_hwreg_read(struct zl3073x_dev *zldev, u32 addr, u32 *value)
{
	int rc;

	/* Set address to read data from */
	rc = zl3073x_write_hwreg_addr(zldev, addr & ~3);
	if (rc)
		return rc;

	/* Perform the read operation */
	rc = zl3073x_hwreg_do_op(zldev, ZL3073X_REG_HWREG_OP_READ);
	if (rc)
		return rc;

	/* Read the received data */
	return zl3073x_read_hwreg_read_data(zldev, value);
}
 ...
 * success. Otherwise returns negative value.
 */
static int zl3073x_hwreg_write(struct zl3073x_dev *zldev, u32 addr, u32 value)
{
	int rc;
	...
	rc = zl3073x_write_hwreg_write_data(zldev, value);
	if (rc)
		return rc;

	/* Perform the write operation */
	return zl3073x_hwreg_do_op(zldev, ZL3073X_REG_HWREG_OP_WRITE);
}
 ...
 */
static int zl3073x_hwreg_update(struct zl3073x_dev *zldev, u32 addr, u32 value,
				u32 mask)
{
	u32 tmp;
	int rc;
 ...
 */
static int zl3073x_hwreg_write_seq(struct zl3073x_dev *zldev,
 ...
 * Returns 0 in case of success or negative value otherwise.
 */
static int zl3073x_flash_download_block(struct zl3073x_dev *zldev,
					struct zl3073x_flash_image *image,
					u32 start, u32 size,
					struct netlink_ext_ack *extack)
{
 ...
 */
static unsigned int zl3073x_flash_get_error_count(struct zl3073x_dev *zldev,
						  unsigned int *causep)
{
	unsigned int count, cause;
	int rc;

	rc = zl3073x_read_error_count(zldev, &count);
	if (rc)
		return UINT_MAX;

	rc = zl3073x_read_error_cause(zldev, &cause);
	if (rc)
		return UINT_MAX;
 ...
 */
static int zl3073x_flash_wait_ready(struct zl3073x_dev *zldev,
...
#define ZL3073X_FLASH_POLL_DELAY_MS	100
	...
	while (time_before(jiffies, timeout)) {
		...
		if (FIELD_GET(ZL3073X_REG_WRITE_FLASH_OP, value) ==
		    ZL3073X_REG_WRITE_FLASH_OP_DONE)
 ...
 */
static int zl3073x_flash_cmd_wait(struct zl3073x_dev *zldev, u32 operation)
{
...
#define FLASH_PHASE2_TIMEOUT_MS 120000	/* up to 2 minutes */
	...
	rc = zl3073x_read_write_flash(zldev, &tmp);
	if (rc) {
		dev_err(zldev->dev,
			"Failed to read write flash register: %d\n", rc);
		return rc;
	}
	
	tmp &= ~ZL3073X_REG_WRITE_FLASH_OP;
	tmp |= operation & ZL3073X_REG_WRITE_FLASH_OP;

	rc = zl3073x_write_write_flash(zldev, tmp);
	if (rc) {
		dev_err(zldev->dev,
	...
	if (rc) {
		dev_err(zldev->dev,
			"Failed to finish flash operation: %d\n", rc);
		return rc;
	}

	/* Check operation status */
	rc = zl3073x_read_op_state(zldev, &state);
	if (rc) {
		dev_err(zldev->dev,
			"Failed to check operation status: %d\n", rc);
		return rc;
	}

	/* Check if the operation was done */
	if (state != ZL3073X_REG_OP_STATE_DONE) {
		dev_err(zldev->dev,
 ...
 * Returns 0 in case of success or negative value otherwise.
 */
static int zl3073x_flash_get_sector_size(struct zl3073x_dev *zldev,
					 u32 *sector_size)
{
	unsigned char flash_info;
	int rc;

	rc = zl3073x_read_flash_info(zldev, &flash_info);
	if (rc)
		return rc;

	switch (FIELD_GET(ZL3073X_REG_FLASH_INFO_SECTOR_SIZE, flash_info)) {
	case ZL3073X_REG_FLASH_INFO_SECTOR_4K:
		*sector_size = 0x1000;
		break;
	case ZL3073X_REG_FLASH_INFO_SECTOR_64K:
		*sector_size = 0x10000;
		break;
	default:
		rc = -EINVAL;
...
#define ZL3073X_FLASH_PAGE_SIZE		0x40 /* in dwords - 256 bytes*/
	...
	for (idx = 0; idx < image->nwords; idx += bsize) {
		...
		rc = zl3073x_write_image_start_addr(zldev,
		...
		rc = zl3073x_write_flash_index_write(zldev, page);
		if (rc)
			goto finish;

		/* Set filling pattern */
		rc = zl3073x_write_fill_pattern(zldev, U32_MAX);
		if (rc)
			goto finish;
			      ...
			      struct netlink_ext_ack *extack)
{
	...
	/* Set address to flash from */
	rc = zl3073x_write_image_start_addr(zldev, image->type->load_addr);
	if (rc)
		goto finish;
	...
	/* Set destination page to flash */
	rc = zl3073x_write_flash_index_write(zldev, image->type->flash_page);
	if (rc)
		goto finish;
	...
	/* Execute sectors flash operation */
	rc = zl3073x_flash_cmd_wait(zldev, ZL3073X_REG_WRITE_FLASH_OP_PAGE);
	if (rc)
		goto finish;
					...
					struct netlink_ext_ack *extack)
{
	...
	rc = zl3073x_write_flash_index_read(zldev, image->type->flash_page);
	if (rc)
		return rc;

	/* Set destination page for the copy */
	rc = zl3073x_write_flash_index_write(zldev, image->type->copy_page);
	if (rc)
		return rc;

	/* Perform copy operation */
	rc = zl3073x_flash_cmd_wait(zldev,
				    ZL3073X_REG_WRITE_FLASH_OP_COPY_PAGE);
	if (rc)
		FLASH_ERR_MSG(zldev, extack,
 ...
 */
static int zl3073x_flash_check_utility(struct zl3073x_dev *zldev)
{
	unsigned char fam, rel;
	unsigned int hash;
	int rc;

	rc = zl3073x_read_flash_hash(zldev, &hash);
	if (rc)
		return rc;

	rc = zl3073x_read_flash_fam(zldev, &fam);
	if (rc)
		return rc;

	rc = zl3073x_read_flash_rel(zldev, &rel);
	if (rc)
		return rc;
				 ...
				 struct netlink_ext_ack *extack)
{
	...
	/* Enable host control */
	rc = zl3073x_read_host_control(zldev, &tmp);
	if (rc) {                                 
		FLASH_ERR_MSG(zldev, extack,
	...
	tmp &= ~ZL3073X_REG_HOST_CONTROL_ENABLE;
	tmp |= ZL3073X_REG_HOST_CONTROL_ENABLE;
	
	rc = zl3073x_write_host_control(zldev, tmp);
	if (rc) {                                 
		FLASH_ERR_MSG(zldev, extack,
