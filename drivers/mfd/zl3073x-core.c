...
#define ZL_POLL_TIMEOUT_US 2000000
	...
	return regmap_read_poll_timeout(zldev->regmap, ZL_REG_ADDR(cmd_reg),
					val, !(val & cmd), ZL_POLL_SLEEP_US,
					ZL_POLL_TIMEOUT_US);
}
			 ...
			 const struct zl3073x_mb_ref *mb)
{
	...
	return zl3073x_mb_cmd_do(zldev, ZL_REG_DPLL_MB_SEM, ZL_REF_MB_SEM_WR,
				 ZL_REG_REF_MB_MASK, BIT(index));
}
