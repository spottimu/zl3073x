...
{
	struct zl3073x_input *input = &zldev->input[index];
	u8 ref_config;
	int rc;
...
{
	struct zl3073x_output *output = &zldev->output[index];
	...
	dev_dbg(zldev->dev, "OUTPUT%u is %s and connected to SYNTH%u\n", index,
		output->enabled ? "enabled" : "disabled", output->synth);
...
zl3073x_synth_state_fetch(struct zl3073x_dev *zldev, u8 index)
{
	...
	dev_dbg(zldev->dev, "SYNTH%u is %s and connected to DPLL%u\n", index,
