				     ...
				     u64 *frequency)
{
	...
	rc = zl3073x_mb_ref_read(zldev, ref_id,
				 ZL3073X_MB_REF_FREQ_BASE |
				 ZL3073X_MB_REF_FREQ_MULT |
				 ZL3073X_MB_REF_RATIO_M |
				 ZL3073X_MB_REF_RATIO_N, &mb);
	if (rc)
		return rc;
	...
	*frequency = mul_u64_u32_div(mb.freq_base * mb.freq_mult, mb.ratio_n,
				     mb.ratio_n);
				 ...
				 struct netlink_ext_ack *extack)
{
	...
	/* Read reference configuration into mailbox */
	rc = zl3073x_mb_ref_read(zldev, ref_id,
				 ZL3073X_MB_REF_SYNC_CTRL |
				 ZL3073X_MB_REF_ESYNC_DIV, &mb);
	if (rc)
		return rc;

	sync_mode = FIELD_GET(ZL_REF_SYNC_CTRL_MODE, mb.sync_ctrl);

	switch (sync_mode) {
	case ZL_REF_SYNC_CTRL_MODE_50_50_ESYNC_25_75:
		esync->freq = (mb.esync_div == ZL_REF_ESYNC_DIV_1HZ) ? 1 : 0;
		esync->pulse = 25;
		break;
	default:
		esync->freq = 0;
				 ...
				 struct netlink_ext_ack *extack)
{
	...
	/* Read reference configuration into mailbox */
	rc = zl3073x_mb_ref_read(zldev, ref_id, ZL3073X_MB_REF_SYNC_CTRL, &mb);
	if (rc)
		return rc;
	...
	mask = ZL3073X_MB_REF_SYNC_CTRL;
	mb.sync_ctrl &= ZL_REF_SYNC_CTRL_MODE;
	mb.sync_ctrl |= FIELD_PREP(ZL_REF_SYNC_CTRL_MODE, sync_mode);

	if (freq) {
		/* 1 Hz is only supported frequnecy currently */
		mask |= ZL3073X_MB_REF_ESYNC_DIV;
		mb.esync_div = ZL_REF_ESYNC_DIV_1HZ;
	}
				     ...
				     struct netlink_ext_ack *extack)
{
	...
	rc = zl3073x_dpll_input_ref_frequency_factorize(frequency,
							&mb.freq_base,
							&mb.freq_mult);
	if (rc)
		return rc;

	/* Set FEC ratio to 1:1 */
	mb.ratio_m = 1;
	mb.ratio_n = 1;
...
zl3073x_dpll_selected_ref_set(struct zl3073x_dpll *zldpll, u8 ref)
{
	...
	rc = zl3073x_write_reg(zldev, ZL_REG_DPLL_MODE_REFSEL(zldpll->id),
			       &mode_refsel);
	if (rc)
		return rc;
...
zl3073x_dpll_connected_ref_get(struct zl3073x_dpll *zldpll, u8 *ref)
{
	...
	if (ZL3073X_DPLL_REF_IS_VALID(*ref)) {
		...
		rc = zl3073x_read_reg(zldev, ZL_REG_REF_MON_STATUS(*ref),
				      &ref_status);
		if (rc)
			return rc;
					...
					struct netlink_ext_ack *extack)
{
	...
	rc = zl3073x_read_reg(zldev, ZL_REG_REF_MON_STATUS(ref_id),
			      &ref_status);
	if (rc)
		return rc;
					...
					struct netlink_ext_ack *extack)
{
	...
	/* Read reference configuration into mailbox */
	rc = zl3073x_mb_ref_read(zldev, ref_id,
				 ZL3073X_MB_REF_PHASE_OFFSET_COMP, &mb);
	if (rc)
		return rc;

	/* Perform sign extension for 48bit signed value */
	phase_comp = sign_extend64(mb.phase_offset_comp, 47);
					...
					struct netlink_ext_ack *extack)
{
	 ...
	 * of requested value.
	 */
	mb.phase_offset_comp = (s64) -phase_adjust;

	/* Update reference configuration from mailbox */
	return zl3073x_mb_ref_write(zldev, zl3073x_dpll_pin_index_get(pin),
				    ZL3073X_MB_REF_PHASE_OFFSET_COMP, &mb);
}
...
zl3073x_dpll_ref_prio_get(struct zl3073x_dpll_pin *pin, u8 *prio)
{
	...
	prio_id = zl3073x_dpll_pin_index_get(pin) / 2;

	/* Read DPLL configuration into mailbox */
	rc = zl3073x_mb_dpll_read(zldev, zldpll->id,
				  ZL3073X_MB_DPLL_REF_PRIO(prio_id), &mb);
	if (rc)
		return rc;

	/* Select nibble according pin type */
	if (zl3073x_dpll_is_p_pin(pin))
		*prio = FIELD_GET(ZL_DPLL_REF_PRIO_REF_P, mb.ref_prio[prio_id]);
	else
		*prio = FIELD_GET(ZL_DPLL_REF_PRIO_REF_N, mb.ref_prio[prio_id]);
...
zl3073x_dpll_ref_prio_set(struct zl3073x_dpll_pin *pin, u8 prio)
{
	...
	/* Read DPLL configuration into mailbox */
	rc = zl3073x_mb_dpll_read(zldev, zldpll->id,
				  ZL3073X_MB_DPLL_REF_PRIO(idx), &mb);
	if (rc)
		return rc;

	/* Update nibble according pin type */
	if (zl3073x_dpll_is_p_pin(pin)) {
		mb.ref_prio[idx] &= ~ZL_DPLL_REF_PRIO_REF_P;
		mb.ref_prio[idx] |= FIELD_PREP(ZL_DPLL_REF_PRIO_REF_P, prio);
	} else {
		mb.ref_prio[idx] &= ~ZL_DPLL_REF_PRIO_REF_N;
		mb.ref_prio[idx] |= FIELD_PREP(ZL_DPLL_REF_PRIO_REF_N, prio);
	}

	/* Update configuration from mailbox */
				  ...
				  struct netlink_ext_ack *extack)
{
	...
	rc = zl3073x_mb_output_read(zldev, output,
				    ZL3073X_MB_OUTPUT_MODE |
				    ZL3073X_MB_OUTPUT_DIV |
				    ZL3073X_MB_OUTPUT_ESYNC_PERIOD |
				    ZL3073X_MB_OUTPUT_ESYNC_WIDTH,
				    &mb);
	if (rc)
		return rc;
	...
	clock_type = FIELD_GET(ZL_OUTPUT_MODE_CLOCK_TYPE, mb.mode);
	 ...
	 * esync_freq = synth_freq / mb.div / mb.esync_period
	 *     ...    = synth_freq / (mb.div * mb.esync_period)
	 */
	esync->freq = div64_u64(synth_freq,
				mul_u32_u32(mb.div, mb.esync_period));
	 ...
	 */
	esync->pulse = (50 * mb.esync_width) / mb.div;

finish:
	/* Set supported esync ranges if the pin supports esync control and
	 * if the output frequency is > 1 Hz.
	 */
	if (pin->esync_control && div_u64(synth_freq, mb.div) > 1) {
		esync->range = esync_freq_ranges;
		esync->range_num = ARRAY_SIZE(esync_freq_ranges);
	} else {
		esync->range = NULL;
				  ...
				  struct netlink_ext_ack *extack)
{
	...
	mask = ZL3073X_MB_OUTPUT_MODE;
	mb.mode &= ~ZL_OUTPUT_MODE_CLOCK_TYPE;
	mb.mode |= FIELD_PREP(ZL_OUTPUT_MODE_CLOCK_TYPE, clock_type);
	...
	mask |= ZL3073X_MB_OUTPUT_ESYNC_PERIOD;
	mb.esync_period = (u32)div64_u64(synth_freq, freq * mb.div);
	 ...
	 * lost.
	 */
	mask |= ZL3073X_MB_OUTPUT_ESYNC_WIDTH;
	mb.esync_width = mb.div / 2;

write_mailbox:
	/* Update output configuration from mailbox */
	return zl3073x_mb_output_write(zldev, output, mask, &mb);
}
				      ...
				      struct netlink_ext_ack *extack)
{
	...
	rc = zl3073x_mb_output_read(zldev, output,
				    ZL3073X_MB_OUTPUT_DIV |
				    ZL3073X_MB_OUTPUT_ESYNC_PERIOD, &mb);
	if (rc)
		return rc;
	...
	case ZL_OUTPUT_MODE_SIGNAL_FORMAT_2_NDIV_INV:
		...
		if (zl3073x_dpll_is_p_pin(pin)) {
			 ...
			 * divisor.
			 */
			*frequency = div_u64(synth_freq, mb.div);
		} else {
			...
			divisor = mul_u32_u32(mb.div, mb.esync_period);
				      ...
				      struct netlink_ext_ack *extack)
{
	...
	if (signal_format != ZL_OUTPUT_MODE_SIGNAL_FORMAT_2_NDIV &&
		 ...
		 */
		mask = ZL3073X_MB_OUTPUT_DIV;
		mb.div = new_div;

		/* For 50/50 duty cycle the divisor is equal to width */
		mask |= ZL3073X_MB_OUTPUT_WIDTH;
		mb.width = new_div;
	...
	output_p_freq = (u32)div_u64(synth_freq, mb.div);

	/* Compute current N-pin frequency */
	output_n_freq = output_p_freq / mb.esync_period;
	...
	if (zl3073x_dpll_is_p_pin(pin)) {
		...
		/* Update the divisor in mailbox with new divisor */
		mask = ZL3073X_MB_OUTPUT_DIV;
		mb.div = new_div;
		...
		mask |= ZL3073X_MB_OUTPUT_ESYNC_PERIOD;
		mb.esync_period = (u32)div_u64(frequency, output_n_freq);
	} else {
		...
		/* Compute new divisor for N-pin */
		mask = ZL3073X_MB_OUTPUT_ESYNC_PERIOD;
		mb.esync_period = output_p_freq / (u32)frequency;
	}

	/* For 50/50 duty cycle the divisor is equal to width */
	mask |= ZL3073X_MB_OUTPUT_ESYNC_WIDTH;
	mb.esync_width = mb.esync_period;
					 ...
					 struct netlink_ext_ack *extack)
{
	...
	/* Read output configuration into mailbox */
	rc = zl3073x_mb_output_read(zldev, output, ZL3073X_MB_OUTPUT_PHASE_COMP,
				    &mb);
	if (rc)
		return rc;

	/* Value in register is expressed in half synth clock cycles */
	phase_comp = (s32)mb.phase_comp;
					 ...
					 struct netlink_ext_ack *extack)
{
	 ...
	 * of requested value.
	 */
	mb.phase_comp = -phase_adjust;

	/* Update output configuration from mailbox */
	output = zl3073x_dpll_output_pin_output_get(pin);
	return zl3073x_mb_output_write(zldev, output,
				       ZL3073X_MB_OUTPUT_PHASE_COMP, &mb);
}
			     ...
			     struct netlink_ext_ack *extack)
{
	...
	rc = zl3073x_read_reg(zldev, ZL_REG_DPLL_MON_STATUS(zldpll->id),
			      &mon_status);
...
zl3073x_dpll_register(struct zl3073x_dpll *zldpll)
{
	...
	rc = zl3073x_read_reg(zldev, ZL_REG_DPLL_MODE_REFSEL(zldpll->id),
			      &dpll_mode_refsel);
	if (rc)
		return rc;
...
zl3073x_dpll_periodic_work(struct kthread_work *work)
{
	...
	if (rc) {
		dev_err(zldev->dev,
			"Failed to get DPLL%u lock status: %pe\n", zldpll->id,
			ERR_PTR(rc));
		goto out;
	}
	...
	rc = zl3073x_mb_phase_meas_do(zldev, zldpll->id, &mb_phase);
	if (rc) {
		dev_err(zldev->dev,
...
zl3073x_dpll_init_fine_phase_adjust(struct zl3073x_dpll *zldpll)
{
	...
	val = 0x1f;
	rc = zl3073x_write_reg(zldev, ZL_REG_SYNTH_PHASE_SHIFT_MASK, &val);
	if (rc)
		return rc;

	val = 0x01;
	rc = zl3073x_write_reg(zldev, ZL_REG_SYNTH_PHASE_SHIFT_INTVL, &val);
	if (rc)
		return rc;

	val = 0xffff;
	rc = zl3073x_write_reg(zldev, ZL_REG_SYNTH_PHASE_SHIFT_DATA, &val);
	if (rc)
		return rc;

	val = 0x01;
	rc = zl3073x_write_reg(zldev, ZL_REG_SYNTH_PHASE_SHIFT_CTRL, &val);
	if (rc)
		return rc;
