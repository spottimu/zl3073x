				     ...
				     u64 *frequency)
{
	...
	/* Read base frequency */
	rc = zl3073x_mb_read_ref_freq_base(zldev, &base_freq);
	if (rc)
		return rc;

	/* Read multiplier */
	rc = zl3073x_mb_read_ref_freq_mult(zldev, &mult);
	if (rc)
		return rc;

	/* Write numerator */
	rc = zl3073x_mb_read_ref_ratio_m(zldev, &num);
	if (rc)
		return rc;

	/* Write denominator */
	rc = zl3073x_mb_read_ref_ratio_n(zldev, &denom);
	if (rc)
		return rc;
				 ...
				 struct netlink_ext_ack *extack)
{
	...
	scoped_guard(zl3073x_mailbox, zldev) {
		/* Read reference configuration into mailbox */
		rc = zl3073x_mb_ref_read(zldev, ref_id);
		if (rc)
			return rc;
		...
		/* Get ref sync mode */
		rc = zl3073x_mb_read_ref_sync_ctrl(zldev, &ref_sync_ctrl);
		if (rc)
			return rc;
		...
		/* Get esync divisor */
		rc = zl3073x_mb_read_ref_esync_div(zldev, &esync_div);
		if (rc)
			return rc;
	}
				 ...
				 struct netlink_ext_ack *extack)
{
	...
	rc = zl3073x_mb_write_ref_sync_ctrl(zldev, ref_sync_ctrl);
	if (rc)
		return rc;

	if (freq) {
		/* 1 Hz is only supported frequnecy currently */
		rc = zl3073x_mb_write_ref_esync_div(zldev,
				     ...
				     struct netlink_ext_ack *extack)
{
	...
	rc = zl3073x_mb_write_ref_freq_base(zldev, base_freq);
	if (rc)
		return rc;

	/* Write multiplier */
	rc = zl3073x_mb_write_ref_freq_mult(zldev, mult);
	if (rc)
		return rc;

	/* Write numerator */
	rc = zl3073x_mb_write_ref_ratio_m(zldev, 1);
	if (rc)
		return rc;

	/* Write denominator */
	rc = zl3073x_mb_write_ref_ratio_n(zldev, 1);
	if (rc)
		return rc;
					...
					struct netlink_ext_ack *extack)
{
	...
	/* Although DPLL measurement registers are not placed in mailbox
	 * pages we have to access these registers in this scope atomically.
	 */
	scoped_guard(zl3073x_mailbox, zldev) {
		...
		/* Update measurement control register with new value */
		rc = zl3073x_write_dpll_meas_ctrl(zldev, dpll_meas_ctrl);
		if (rc)
			return rc;

		/* Set measurement index to channel index */
		dpll_meas_idx = FIELD_PREP(ZL_DPLL_MEAS_IDX, zldpll->id);
		...
		rc = zl3073x_poll_ref_phase_err_read_rqst(zldev,
							  ZL_REF_PHASE_ERR_READ_RQST_RD);
		if (rc)
			return rc;
					...
					struct netlink_ext_ack *extack)
{
	...
	rc = zl3073x_mb_read_ref_phase_compensation(zldev, &phase_comp);
	if (rc)
		return rc;
					...
					struct netlink_ext_ack *extack)
{
	...
	rc = zl3073x_mb_write_ref_phase_compensation(zldev, phase_comp);
	if (rc)
		return rc;
...
zl3073x_dpll_ref_prio_get(struct zl3073x_dpll_pin *pin, u8 *prio)
{
	...
	rc = zl3073x_mb_read_dpll_ref_prio(zldev, ref_id / 2, &ref_prio);
	if (rc)
		return rc;
...
zl3073x_dpll_ref_prio_set(struct zl3073x_dpll_pin *pin, u8 prio)
{
	...
	/* Read ref prio nibble */
	rc = zl3073x_mb_read_dpll_ref_prio(zldev, idx / 2, &ref_prio);
	if (rc)
		return rc;
	...
	rc = zl3073x_mb_write_dpll_ref_prio(zldev, idx / 2, ref_prio);
	if (rc)
		return rc;
				  ...
				  struct netlink_ext_ack *extack)
{
	...
	case ZL_OUTPUT_SIGNAL_FORMAT_TWO_N_DIV:
	case ZL_OUTPUT_SIGNAL_FORMAT_TWO_N_DIV_INV:
		return -EOPNOTSUPP;
	default:
		break;
	}
	...
	/* Read output divisor */
	rc = zl3073x_mb_read_output_div(zldev, &output_div);
	if (rc)
		return rc;
	...
	/* Read esync period */
	rc = zl3073x_mb_read_output_esync_period(zldev, &esync_period);
	if (rc)
		return rc;
	...
	rc = zl3073x_mb_read_output_esync_width(zldev, &esync_width);
	if (rc)
		return rc;
				  ...
				  struct netlink_ext_ack *extack)
{
	...
	rc = zl3073x_mb_write_output_mode(zldev, output_mode);
	if (rc)
		return rc;
	...
	rc = zl3073x_mb_write_output_esync_period(zldev, esync_period);
	if (rc)
		return rc;
	...
	rc = zl3073x_mb_write_output_esync_width(zldev, esync_width);
	if (rc)
		return rc;
	...
	case ZL_OUTPUT_SIGNAL_FORMAT_TWO_N_DIV_INV:
		...
		if (zl3073x_dpll_is_p_pin(pin)) {
			...
			*frequency = div_u64(synth_freq, output_div);
			...
			rc = zl3073x_mb_read_output_ndiv_period(zldev, &ndiv);
			if (rc)
				return rc;
				      ...
				      struct netlink_ext_ack *extack)
{
	...
	/* Check signal format */
	if (signal_format != ZL_OUTPUT_SIGNAL_FORMAT_TWO_N_DIV &&
	    signal_format != ZL_OUTPUT_SIGNAL_FORMAT_TWO_N_DIV_INV) {
		/* For non N-divided signal formats the frequency is computed
		 * as division of synth frequency and output divisor.
		 */
		rc = zl3073x_mb_write_output_div(zldev, new_div);
		if (rc)
			return rc;

		/* For 50/50 duty cycle the divisor is equal to width */
		rc = zl3073x_mb_write_output_width(zldev, new_div);
		if (rc)
			return rc;
	...
	rc = zl3073x_mb_read_output_div(zldev, &cur_div);
	if (rc)
		return rc;
	...
	/* Get N-pin divisor */
	rc = zl3073x_mb_read_output_ndiv_period(zldev, &n_div);
	if (rc)
		return rc;
	...
	rc = zl3073x_mb_write_output_ndiv_period(zldev, n_div);
	if (rc)
		return rc;

	/* For 50/50 duty cycle the divisor is equal to width */
	rc = zl3073x_mb_write_output_ndiv_width(zldev, n_div);
	if (rc)
		return rc;
					 ...
					 struct netlink_ext_ack *extack)
{
	...
	rc = zl3073x_mb_read_output_phase_compensation(zldev, &phase_comp);
	if (rc)
		return rc;
					 ...
					 struct netlink_ext_ack *extack)
{
	...
	/* Write the requested value into the compensation register */
	rc = zl3073x_mb_write_output_phase_compensation(zldev, phase_comp);
	if (rc)
		return rc;
					...
					struct zl3073x_dpll_pin_info *pin_info)
{
	...
	if (zl3073x_dpll_is_input_pin(pin)) {
		...
		case ZL_OUTPUT_SIGNAL_FORMAT_LVDS:
		case ZL_OUTPUT_SIGNAL_FORMAT_DIFFERENTIAL:
		case ZL_OUTPUT_SIGNAL_FORMAT_LOWVCM:
...
zl3073x_dpll_register_output_pin(struct zl3073x_dpll_pin *pin)
{
	...
	case ZL_OUTPUT_SIGNAL_FORMAT_DISABLED:
	...
	case ZL_OUTPUT_SIGNAL_FORMAT_TWO:
	case ZL_OUTPUT_SIGNAL_FORMAT_TWO_INV:
	case ZL_OUTPUT_SIGNAL_FORMAT_TWO_N_DIV:
	case ZL_OUTPUT_SIGNAL_FORMAT_TWO_N_DIV_INV:
	...
	case ZL_OUTPUT_SIGNAL_FORMAT_ONE_P:
		...
		break;
	case ZL_OUTPUT_SIGNAL_FORMAT_ONE_N:
...
zl3073x_dpll_periodic_work(struct kthread_work *work)
{
	...
	for (i = 0; i < ZL3073X_NUM_INPUT_PINS; i++) {
		...
		if (rc) {
			dev_err(zldev->dev,
				"Failed to get INPUT%u on DPLL%u phase offset: %pe\n",
				index, zldpll->id, ERR_PTR(rc));
			goto out;
		}
